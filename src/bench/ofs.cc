...
int main(int argc, char **argv) {
  ...
  for (uint8_t r = 1; r <= c.num_runs_; ++r) {
    ...
    // std::clog << "Creating OFileStore with lf=" << lf << ", s=" <<
    // int(c.num_levels_) << std::endl;
    auto opt_ofs = OFileStore::SConstruct(c.capacity_, c.num_levels_, lf,
                                          c.base_block_size_, ek, chan,
                                          data_store, aux_store, true, true,
                                          c.storage_type_, r, c.initial_level_);
    ...
    for (uint64_t k = 1; k < size_t(c.capacity_) / 4; k <<= 1) {
      ...
      std::clog << "Searched " << k << " (" << searched << ") took "
  ...
  std::cout << "Total memory usage: " << get_mem_usage() - baseline
            << std::endl;
  total /= c.num_runs_;
