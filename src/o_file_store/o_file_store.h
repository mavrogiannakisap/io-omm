...
template <int i> class OMapValMulti32 {
public:
...
public:
  ...
  explicit SuperBlockMeta(uint32_t cnt, bool no_alloc_buffer = false)
      : part_count_(cnt), parts_valid_(0),
        parts_(cnt && !no_alloc_buffer ? std::make_unique<PartMeta[]>(cnt)
                                       : nullptr) {}
  ...
  static size_t SizeInBytes(uint32_t part_count) {
    return sizeof(part_count_) + sizeof(parts_valid_) +
           (uint64_t(part_count) * sizeof(PartMeta));
  }
};
...
public:
  ...
  SuperBlock(char *data, size_t byte_size)
      : byte_size_(byte_size), data_(data), meta_(data) {}
  SuperBlock(std::unique_ptr<char[]> data, size_t byte_size)
      : byte_size_(byte_size), data_(std::move(data)), meta_(data_.get()) {
    my_assert(IsSizeValid());
  }
  SuperBlock(size_t byte_size, uint32_t part_count,
             bool no_alloc_buffer = false)
  ...
  [[nodiscard]] size_t FreeSpace() const {
    return byte_size_ - SuperBlockMeta::SizeInBytes(meta_.part_count_) -
           UsedSpace();
  }
...
private:
  bool IsSizeValid() {
    return SuperBlockMeta::SizeInBytes(meta_.part_count_) < byte_size_;
  }
};
...
static Key ExtractKey(uint64_t key_idx) { return Key(key_idx >> 32); }

static uint32_t ExtractIdx(uint64_t key_idx) { return key_idx; }
...
class NaiveOram {
public:
  static std::optional<NaiveOram>
  Construct(size_t total_cap, uint32_t max_parts, utils::Key enc_key,
            std::shared_ptr<grpc::Channel> channel,
            storage::InitializeRequest_StoreType st, bool first_build = false);
  void Destroy();
  void Fetch();
  void Evict();
...
private:
  NaiveOram(size_t total_cap, uint32_t max_parts, utils::Key enc_key,
            std::shared_ptr<grpc::Channel> channel,
            storage::InitializeRequest_StoreType st, bool first_build = false);
...
class OFileStore {
public:
  static std::optional<OFileStore>
  Construct(uint32_t n, uint8_t s, size_t &lf, uint32_t base_block_size,
            ...
            bool upload_stash = true, bool first_build = false,
            std::string storage_type_ = "RAM", uint8_t init_level_ = 10);
  static std::optional<OFileStore>
  SConstruct(uint32_t n, uint8_t s, size_t &lf, uint32_t base_block_size,
             utils::Key enc_key, const std::shared_ptr<grpc::Channel> &channel,
             storage::InitializeRequest_StoreType data_st,
  ...
  void PrebuildAppend(Key k,
                      Val v); // Adds everything to the stash and evicts once.
  ...
  void ResetAvailablePaths() {
    for (auto &oram_ : orams_)
      oram_->ResetAvailablePaths();
  }
...
private:
  ...
  std::unique_ptr<internal::OMap> size_key_pos_map_ =
      nullptr; // when all levels present.
  std::unique_ptr<internal::OMap> counter_map_ = nullptr;
  std::unique_ptr<internal::NaiveOram> naive_oram_ =
      nullptr; // One is really enough.
  ...
  MakeUniformLevels(size_t n, uint8_t s, size_t &lf);
  [[nodiscard]] static std::vector<uint8_t>
  MakeLevels(size_t n, uint8_t s, std::string storage_type_,
             uint32_t base_block_size_, uint8_t init_level_);
  [[nodiscard]] size_t NumBlocks(size_t n) const {
    return ceil_div(n, base_block_size_);
  }
  [[nodiscard]] uint8_t LevelIdx(uint32_t n) const;
  [[nodiscard]] size_t
  SuperBlockMaxBaseBlocksInVal(uint8_t l) const; // times base_block_size_
  ...
  //  size_t GetAndUpdateSize(Key k, uint32_t n, bool append = false, bool
  //  hide_if_exists = true);
  ...
  [[nodiscard]] internal::PosPair
  GetOldPosAndReposition(internal::ORamKey ok, uint8_t l,
                         bool must_exist = false);
  [[nodiscard]] bool IsLevelNaive(uint8_t l) const;
  void InsertPart(Key k, uint32_t i, internal::Part part, uint8_t l,
                  bool prebuild);
