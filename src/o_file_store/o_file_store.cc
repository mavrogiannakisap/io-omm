...
OptPart SuperBlock::Read(Key k) {
  ...
  auto res_start = data_.get() + SuperBlockMeta::SizeInBytes(meta_.part_count_) + base;
...
OptPart SuperBlock::Remove(Key k) {
  ...
  std::copy_n(meta_.parts_.get() + i + 1, meta_.parts_valid_ - i - 1, meta_.parts_.get() + i);
  --meta_.parts_valid_;
...
bool SuperBlock::Append(Key k, char *from, size_t n) {
  if (meta_.part_count_ != 1
      || (meta_.parts_valid_ == 1 && meta_.parts_[0].k_ != k)
      || FreeSpace() < n) {
    return false;
  }
  auto to = data_.get()
      + SuperBlockMeta::SizeInBytes(1)
...
void SuperBlock::Add(Key k, Val &v) {
  ...
  auto write_base = data_.get() + SuperBlockMeta::SizeInBytes(meta_.part_count_) + UsedSpace();
...
std::optional<NaiveOram> NaiveOram::Construct(
    size_t total_cap, uint32_t max_parts,
    utils::Key enc_key,
    std::shared_ptr<grpc::Channel> channel,
    storage::InitializeRequest_StoreType st,
    bool first_build) {
  NaiveOram o(total_cap, max_parts, enc_key, std::move(channel), st, first_build);
  if (o.setup_successful_) return std::move(o);
  return std::nullopt;
}

NaiveOram::NaiveOram(size_t total_cap, uint32_t max_parts,
                     utils::Key enc_key,
                     std::shared_ptr<grpc::Channel> channel,
                     storage::InitializeRequest_StoreType st,
                     bool first_build)
  ...
  req.set_name(utils::GenName(
      {
          "naive", "",
          "n", std::to_string(ctext_size_),
      }));
  // Only to not recreate with different IDs (the server is not initialized) when pre-building
...
void NaiveOram::Fetch() {
  ...
  auto plen = utils::DecryptStrArray(
      ctext_parts, 0, ctext_size_, enc_key_, sb_.data_.get());
...
void NaiveOram::Evict() {
  if (server_valid_ || !sb_.IsAllocated()) return;
  server_valid_ = true;
  bytes_moved_ += ctext_size_;
  ...
  auto enc_success = utils::Encrypt(sb_.data_.get(), ptext_size_,
                                    enc_key_, ctext);
...
OptVal NaiveOram::ReadAndRemove(Key k) {
  if (server_valid_ || !sb_.IsAllocated()) return std::nullopt;
  auto p = sb_.Remove(k);
...
bool NaiveOram::ReadAndRemoveTo(Key k, char *to) {
  if (server_valid_ || !sb_.IsAllocated()) return false;
  return sb_.RemoveTo(k, to);
}

void NaiveOram::Add(Key k, Val v) {
  if (server_valid_) return;
  if (!server_valid_ && !sb_.IsAllocated()) sb_.EnsureAllocated();
...
std::optional<OFileStore> OFileStore::SConstruct(uint32_t n, uint8_t s, size_t &lf, uint32_t base_block_size,
    utils::Key enc_key,
    const std::shared_ptr<grpc::Channel> &channel,
    storage::InitializeRequest_StoreType data_st,
    storage::InitializeRequest_StoreType aux_st,
...
OFileStore::OFileStore(
  ...
  for (uint8_t l = 0; l < uint8_t(levels_.size()); ++l) {
    if (IsLevelNaive(l)) {
      ...
      auto opt_oram = NaiveOram::Construct(
          uint64_t(n) * uint64_t(base_block_size), n,
    ...
    auto opt_ost = OSegTree::Construct(
        level_cap, SuperBlockNumBaseBlocks(l), enc_key,
    ...
    allocators_.push_back(std::make_unique<OSegTree>(std::move(opt_ost.value())));

    auto opt_oram = ORam::Construct(
        level_cap, SuperBlockSizeWithMetaInBytes(l), enc_key,
        channel, data_st, aux_st, upload_stash, "", first_build);
    if (!opt_oram) {
      Destroy();
      return;
    }
    ...
    auto opt_omap = OMap::Construct(
        level_cap, sizeof(uint32_t), enc_key,
        channel, aux_st, aux_st, upload_stash, first_build);
    if (!opt_omap) {
      Destroy();
      return;
  ...
  if (has_all_levels_) {
    auto opt_omap = OMap::Construct(
        capacity_, sizeof(SKPOMapVal), enc_key,
        channel, aux_st, aux_st, upload_stash, first_build);
    if (!opt_omap) {
      Destroy();
      return;
  ...
  auto opt_omap = OMap::Construct(
      capacity_, sizeof(uint32_t), enc_key,
      channel, aux_st, aux_st, upload_stash, first_build);
  if (!opt_omap) {
    Destroy();
    return;
  ...
  was_prebuilt_ &= opt_omap->WasPrebuilt();
  opt_omap->FillWithDummies();
  size_map_ = std::make_unique<OMap>(std::move(opt_omap.value())); //length omap

  auto opt_omap2 = OMap::Construct(
      capacity_, sizeof(uint32_t), enc_key,
      channel, aux_st, aux_st, upload_stash, first_build);
  if (!opt_omap2) {
    Destroy();
  ...
  part_oram_key_map_ = std::make_unique<OMap>(std::move(opt_omap2.value())); // block id omap

  // Added by TOLIS:
  auto opt_omap3 = OMap::Construct(
      capacity_, sizeof(uint32_t), enc_key,
      channel, aux_st, aux_st, upload_stash, first_build);
  if (!opt_omap3) {
    Destroy();
...
OFileStore::OFileStore(
    uint32_t n, uint8_t s, uint32_t base_block_size,
    utils::Key enc_key,
    const std::shared_ptr<grpc::Channel> &channel,
    storage::InitializeRequest_StoreType data_st,
    storage::InitializeRequest_StoreType aux_st,
...
std::vector<uint8_t> OFileStore::MakeLevels(size_t n, uint8_t s,
  ...
  if (!storage_type_.compare("RAM")) {
    ...
    for(int i = 0; i < s; i++) {
      res.push_back(floor(log2(int(4096/EncryptedBucketSize(base_block_size_))))+step);
    }
...
std::vector<uint8_t> OFileStore::MakeUniformLevels(size_t n, uint8_t s, size_t &lf) {
  std::vector<uint8_t> res;
  uint8_t l;
  int p;
  my_assert(s <= lg(n) + 1);
  if ((lg(n) + 1) % s == 0) {
    ...
    // scenario 2: Keep level l and 0 unless s=1, and divide the range in between
    l = lg(n / lf);
    p = l / (s - 1);
  } else {
    std::clog << "Bad config; could not make levels. n=" << n
              << ", lf=" << lf << ", s=" << int(s) << std::endl;
    return {};
  }
  ...
  if (lf >= max_lf) {
    ...
    for (const auto &level : res) std::clog << level << " ";
...
uint8_t OFileStore::LevelIdx(uint32_t n) const { // n in base blocks
  ...
  for (int i = 0; i < levels_.size() - 1; ++i) {
    if (n <= uint64_t(locality_factor_) * uint64_t(SuperBlockMaxBaseBlocksInVal(i))) {
      return i;
    }
  }
...
size_t OFileStore::SuperBlockMaxBaseBlocksInVal(uint8_t l) const { // times base_block_size_
  return 1 << levels_[l];
}
...
void OFileStore::Destroy() {
  if (part_oram_key_map_) part_oram_key_map_->Destroy();
  if (size_map_) size_map_->Destroy();
  if (naive_oram_) naive_oram_->Destroy();
  if (size_key_pos_map_) size_key_pos_map_->Destroy();
  if (counter_map_) counter_map_->Destroy();
  for (auto &o : orams_) if (o) o->Destroy();
  for (auto &o : allocators_) if (o) o->Destroy();
  for (auto &o : pos_maps_) if (o) o->Destroy();
}

//size_t OFileStore::GetAndUpdateSize(Key k, uint32_t n, bool append, bool hide_if_exists) {
...
PosPair OFileStore::GetOldPosAndReposition(ORamKey ok, uint8_t l, bool must_exist) {
  auto opt_pos = must_exist ? pos_maps_[l]->Read(ok)
                            : pos_maps_[l]->ReadAndRemove(ok);
...
Part OFileStore::ExtractPartOrFail(Key k, uint32_t i, uint8_t l) {
  ...
  auto sb = SuperBlock(opt_sb.value().release(), SuperBlockSizeWithMetaInBytes(l));
...
void OFileStore::InsertPart(Key k, uint32_t i, Part part, uint8_t l, bool prebuild) {
...
void OFileStore::PrebuildAppend(Key k, Val v) {
  ...
  size_t offset = 0;
  size_t new_max_val_bytes = size_t(SuperBlockMaxBaseBlocksInVal(level)) * (base_block_size_);
...
void OFileStore::AppendSingleLevel(Key k, Val v) {
  ...
  /*
    ...
    curr_num_parts: ceil(l * bbs / B * bbs), if float(l/B) == 0, then lists are full
    In case that's true perform a dummy access
  */

  uint8_t level = 0;

  auto curr_size = opt_curr_size ? OMapVal32(opt_curr_size->get()).v_ : 0;
  auto curr_num_parts = ceil_div(curr_size, SuperBlockMaxBaseBlocksInVal(level));
...
void OFileStore::Append(Key k, Val v, bool optimized) {
  ...
  auto curr_num_parts = ceil_div(curr_size, SuperBlockMaxBaseBlocksInVal(curr_level));
  // my_assert(curr_num_parts <= locality_factor_);
  auto new_size = curr_size + bl_count;
  auto new_level = LevelIdx(new_size);
  auto new_num_parts = ceil_div(new_size, SuperBlockMaxBaseBlocksInVal(new_level));
  ...
  size_t curr_max_val_bytes = size_t(SuperBlockMaxBaseBlocksInVal(curr_level)) * (base_block_size_);
  size_t new_max_val_bytes = size_t(SuperBlockMaxBaseBlocksInVal(new_level)) * (base_block_size_);
  size_t offset = 0;
  for (uint32_t i = 0; i < new_num_parts; ++i) {
    ...
    while (new_part_ptr < new_part_last) {
      ...
      if (offset < curr_bytes) {
        ...
        auto curr_part_ptr = curr_part.data_.get() + (offset % curr_max_val_bytes);
        auto curr_part_last = curr_part.data_.get() + curr_part.meta_.l_;
        my_assert(curr_part_ptr < curr_part_last);
        to_move = min(new_part_last - new_part_ptr, curr_part_last - curr_part_ptr);
...
bool OFileStore::AllLevelsAppend(Key k, Val v) {
  ...
  if (curr_size > 0 && IsLevelNaive(curr_level)) {
    ...
    auto sb = SuperBlock(opt_ov->release(), SuperBlockSizeWithMetaInBytes(curr_level));
    auto removed = sb.RemoveTo(k, new_v.data_.get());
    my_assert(removed);
  }

  std::copy_n(v.data_.get(), v.l_, new_v.data_.get() + (curr_size * base_block_size_));
  ...
  if (!IsLevelNaive(new_level)) {
    ...
    auto opt_ok = allocators_[new_level]->Alloc(SuperBlockMaxBaseBlocksInVal(new_level));
    my_assert(opt_ok.has_value());
    ok = opt_ok.value();
...
void OFileStore::ReadUpdate(Key k, const ValUpdateFunc &val_updater) {
  ...
  // std::clog << "\tcurr_size: " << curr_size << "\n\tcurr_level: " << curr_level << "\n\tcurr_num_parts: " 
            // << curr_num_parts << std::endl;
  if (curr_size > 0) {
    // if (IsLevelNaive(curr_level)) {
    //   naive_oram_->Fetch();
    //   curr_v = std::move(naive_oram_->ReadAndRemove(k));
  ...
  //     naive_oram_->Fetch();
  //   }
  //   naive_oram_->Add(k, std::move(new_val));
  //   if (!manual_evict_) {
  //     naive_oram_->Evict();
  //   }
  //   size_map_->Insert(k, OMapVal32(new_size).ToBytes());
  ...
  auto new_num_parts = ceil_div(new_size, SuperBlockMaxBaseBlocksInVal(new_level));
  // my_assert(new_num_parts <= locality_factor_);
  size_t new_sb_bytes = SuperBlockMaxBaseBlocksInVal(new_level) * base_block_size_;
...
void OFileStore::Search(Key k, const ValUpdateFunc &val_updater) {
  ...
  for (uint32_t i = 0; i < curr_num_parts; ++i) {
    ...
    my_assert(opt_sb.has_value());
    auto sb = SuperBlock(opt_sb.value().release(), SuperBlockSizeWithMetaInBytes(curr_level));
  ...
  auto new_val = val_updater(std::move(curr_v));
  size_t new_sb_bytes = SuperBlockMaxBaseBlocksInVal(curr_level) * base_block_size_;
  auto new_val_ptr = new_val.data_.get();
  const auto new_val_last = new_val_ptr + new_val.l_;
  for (uint32_t i = 0; i < curr_num_parts; ++i) {
    my_assert(new_val_ptr < new_val_last);
  ...
  now = klock::now();
  res = now - last_read;
  // std::clog << "Time to evict all auxiliary data structures: " << res.count() << std::endl;
...
void OFileStore::AllLevelsReadUpdate(Key k, const ValUpdateFunc &val_updater) {
  ...
  if (curr_size > 0 && IsLevelNaive(curr_level)) {
    ...
    my_assert(opt_ov.has_value());
    SuperBlock sb(opt_ov.value().release(), SuperBlockSizeWithMetaInBytes(curr_level));
    auto opt_part = sb.Remove(k);
    my_assert(opt_part);
...
size_t OFileStore::TotalSizeOfStore() const {
  size_t res = 0;
  if (part_oram_key_map_) res += part_oram_key_map_->TotalSizeOfStore();
  if (size_map_) res += size_map_->TotalSizeOfStore();
  if (size_key_pos_map_) res += size_key_pos_map_->TotalSizeOfStore();
  if (naive_oram_) res += naive_oram_->TotalSizeOfStore();
  for (auto &o : orams_) if (o) res += o->TotalSizeOfStore();
  for (auto &o : allocators_) if (o) res += o->TotalSizeOfStore();
  for (auto &o : pos_maps_) if (o) res += o->TotalSizeOfStore();
  return res;
}
...
bool OFileStore::IsLevelNaive(uint8_t l) const {
  ...
  auto naive_oram_access = SuperBlockMeta::SizeInBytes(NaiveORamMaxPartCount(l))
...
size_t OFileStore::NaiveORamMaxPartCount(uint8_t l) const {
  auto smallest_size_for_level =
      l > 0 ? (uint64_t(locality_factor_) * uint64_t(SuperBlockMaxBaseBlocksInVal(l - 1))) + 1ULL
            : uint64_t(capacity_);
...
size_t OFileStore::BytesMoved() const {
  size_t res = 0;
  if (part_oram_key_map_) res += part_oram_key_map_->BytesMoved();
  if (size_map_) res += size_map_->BytesMoved();
  if (size_key_pos_map_) res += size_key_pos_map_->BytesMoved();
  if (naive_oram_) res += naive_oram_->BytesMoved();
  for (auto &o : orams_) if (o) res += o->BytesMoved();
...
void OFileStore::EvictAll() {
  if (part_oram_key_map_) part_oram_key_map_->EvictAll();
  if (size_map_) size_map_->EvictAll();
  if (size_key_pos_map_) size_key_pos_map_->EvictAll();
  if (naive_oram_) naive_oram_->Evict();
  for (auto &o : orams_) if (o) o->EvictAll();
  for (auto &o : pos_maps_) if (o) o->EvictAll();
}
} // namespace file_oram::o_file_store
