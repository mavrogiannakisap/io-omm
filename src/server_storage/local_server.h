...
private:
    ...
    std::string name;
public:
    Server(std::string file_path, uint32_t n, uint32_t entry_size, std::string name) : n(n), entry_size_(entry_size){
      file_.open(file_path, std::ios::binary | std::ios::in | std::ios::out);
      std::clog << "Initializing ORam, name=" << name << " n= " << n << " bucket_size=" << entry_size_ <<" path= " << file_path <<std::endl;
      if(!file_) {
          std::clog << "[LOCAL SERVER] ERROR: Initialization - cannot open '" << file_path << "'" << std::endl;
          std::exit(1);
      }
    }
    ...
    void Write(char *bytes, uint32_t index) {
        ...
        if(file_.fail()) {
            std::clog << "[LOCAL SERVER] ERROR: failed to write in Write(char *bytes, uint32_t index\n";
        }
    }
    ...
    void WriteMany(const char *bytes, std::vector<uint32_t> idxs) {
        ...
        for(auto &idx : idxs) {
            ...
            if(file_.fail()) {
                std::clog << "[LOCAL SERVER] ERROR: failed to write in WriteMany(char *bytes, vector<> indexes)\n";
            }
            delete[] out;
            offset += entry_size_;
        }
