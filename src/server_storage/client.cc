...
int main() {
    ...
    */
    ...
    std::cout << "Key: " << blocks[0].meta_.key_ << "\tVal:" << blocks[0].val_ << std::endl;
    ...
    for(Key k = 3; k < 6; k++) {
        Block bp;
        server.Read((k - 1) * Block::BlockSize(val_len), Block::BlockSize(val_len), bp);

        std::cout << "k:" << k <<"\tbp.key:" << bp.meta_.key_ << ", bp.val:" << bp.val_ << std::endl;
    }
    ...
    for( auto &b : res) {
        std::cout <<"bp.key:" << b.meta_.key_ << ", bp.val:" << b.val_ << std::endl;
    }
    ...
    for(Key k = 3; k < 6; k++) {
        Block bp;
        rserver.Read(k-1, bp);
        std::cout << "RAMServer: Response (" << bp.meta_.key_ << ", " << bp.val_.get() << ")\n";
    }
